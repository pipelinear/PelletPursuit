...
public class GameApp extends Application {
    ...
    // Canvas and stage stored as fields so startLevel() can resize them for
    // variable-size maps
    ...
    // --- Game state ---
    private enum State {
        READY, GET_READY, PLAYING, DEAD_PAUSE, LEVEL_CLEAR, GAME_OVER, WIN
    }
    ...
    private void startLevel() {
        ...
        // Stagger ghost spawns — ghosts 1-3 start inactive and are released one per
        // spawnDelay
        nextToSpawn = 1;
        spawnTimer = 0;
        if (config.spawnDelay > 0) {
            for (int i = 1; i < ghosts.size(); i++)
                ghosts.get(i).setActive(false);
        }
        dotsEaten = 0;
        bonusItems.clear();
        scoreFlashes.clear();
    ...
    private void updatePlaying(double dt) {
        ...
        if (earned > 0) {
            ...
            if (earned == 50) { // power pellet
                audio.playPellet();
                for (Ghost g : ghosts)
                    g.frighten(config.frightenDuration);
                ghostsEatenThisPellet = 0;
            } else {
                audio.playChomp();
            }
    ...
    private void render(GraphicsContext gc) {
        ...
        if (state == State.READY) {
            ...
            if (killerGhost != null)
                drawCenteredText(gc, "caught by " + killerGhost.getName(), 16, Color.web("#ff8888"),
                        canvasH() / 2.0 + 20);
        } else if (state == State.GAME_OVER) {
            drawGameOver(gc);
